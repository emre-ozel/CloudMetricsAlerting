...
def main():
    ...
    parser.add_argument("--model", choices=["lgbm", "lr", "both"], default="both")
    ...
    if args.model in ("lr", "both"):
        path = DATA_DIR / "model_lr.pkl"
        if path.exists():
            th = thresholds.get("lr", 0.5)
            results.append(
                evaluate_model(path, X_test, y_test, "LogisticRegression", th)
            )
